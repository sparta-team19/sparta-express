...
class ShipmentRouteDomainServiceTest {
    ...
    Hub seoul = Hub.builder().id(UUID.fromString("03125302-ee89-417e-a043-cf9592c804a3"))
            .address("서울특별시 송파구 송파대로 55").latitude(37.474688).longitude(127.125170)
            .status(HubStatus.ACTIVE).build();
    Hub gyeongi = Hub.builder().id(UUID.fromString("608a593a-eb94-4c39-b23d-d2d1d0e1d64b"))
            .address("경기도 고양시 덕양구 권율대로 570").latitude(37.640558).longitude(126.873834)
            .status(HubStatus.ACTIVE).build();
    Hub sejong = Hub.builder().id(UUID.fromString("b5a7694b-5177-471c-a821-5296f6a3c521"))
            .address("세종특별자치시 한누리대로 2130").latitude(36.479999).longitude(127.289113)
            .status(HubStatus.ACTIVE).build();
    Hub daejon = Hub.builder().id(UUID.fromString("21b1768a-8395-49b1-bdf4-15c2b5962b5e"))
            .address("대전 서구 둔산로 100").latitude(36.350244).longitude(127.384785)
            .status(HubStatus.ACTIVE).build();
    Hub busan = Hub.builder().id(UUID.fromString("5660247e-eef8-471c-a0c9-857c5c894b7c"))
    ...
    List<InterhubRoute> expectedShipmentInterhubRoutes = List.of(
            InterhubRoute.builder().status(InterhubRouteStatus.ACTIVE)
                    .originHub(busan).destinationHub(daejon)
                    .distanceKm(400).estimatedMinutes(240)
                    .build(),
    ...
    void findShipmentRoutes() {
        ...
        Position addressPosition = Position.builder().latitude(36.464739).longitude(127.281971).build();

        when(hubRepo.readAllHubs()).thenReturn(hubList);
        when(hubRepo.readAllInterhubRoutes()).thenReturn(givenInterhubRoutes);

        ShipmentRoute shipmentRoutes = service.findShipmentRoutes(seoul.getId(), addressPosition);


        UUID expectedFinalHubId
                = expectedShipmentInterhubRoutes.get(expectedShipmentInterhubRoutes.size() - 1)
                .getDestinationHubId();
        FinalHubToDestination finalHubToDestination = shipmentRoutes.getFinalHubToDestination();
        ...
        assertThat(finalHubToDestination.getDistanceKm()).isEqualTo(3);// 실시간으로 달라지므로 주의, 테스트시 네이버 검색통해 체크할것
    }
